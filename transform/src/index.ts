...
class Schema {
  ...
  public deps: Schema[] = [];
  public eager: boolean = false; // @json({ eager: true }) -> flat eager parse
}
    ...
    return (sourceFile: ts.SourceFile) => {
      function visit(node: ts.Node): ts.Node {
        ...
        // Rewrite JSON.parse<T>(x) / JSON.parse<T[]>(x) into a view construction.
          ...
          const sid = (tn: string) => factory.createPropertyAccessExpression(factory.createIdentifier(`__View_${tn}`), factory.createIdentifier("__sid"));
          const view = (tn: string) => factory.createIdentifier(`__View_${tn}`);
          ...
          if (t.isArrayTypeNode(ta) && t.isTypeReferenceNode(ta.elementType) && t.isIdentifier(ta.elementType.typeName)) {
            const tn = (ta.elementType as ts.TypeReferenceNode).typeName.getText(sourceFile);
            const sch = self.schemas.find((s) => s.name === tn);
            if (sch)
      ...
      if (self.schema) {
        ...
        const synthesize = (n: ts.Node): void => { (n as any).pos = -1; (n as any).end = -1; ts.forEachChild(n, synthesize); };
...
function genView(schema: Schema, all: Schema[]): string {
  const isSchema = (name: string) => all.some((s) => s.name === name);
  const keys: string[] = [];
  const childSids: string[] = [];
  const fields: string[] = [];
  let idx = 0;
  ...
  for (const m of schema.members) {
    ...
    if (ty === "number" || ty === "Number") kind = "num";
    else if (ty === "string" || ty === "String") kind = "str";
    else if (ty === "boolean" || ty === "Boolean") kind = "bool";
    else if (ty.endsWith("[]") || ty.startsWith("Array<")) {
      const elem = (ty.endsWith("[]") ? ty.slice(0, -2) : ty.slice(6, -1)).trim();
      if (elem === "number" || elem === "Number") { kind = "numArray"; child = "__JSONPRIM"; }
      else if (elem === "string" || elem === "String") { kind = "strArray"; child = "__JSONPRIM"; }
      else if (isSchema(elem)) { kind = "structArray"; child = `__View_${elem}.__sid`; childName = elem; }
      else { kind = "numArray"; child = "__JSONPRIM"; }
    } else {
      const base = stripNull(ty).trim();
      if (isSchema(base)) { kind = "child"; child = `__View_${base}.__sid`; childName = base; }
      else kind = "str"; // unknown -> raw string fallback
    }
