...
function commonSourceDirectory(program) {
    const files = program.getSourceFiles().filter((file) => !file.isDeclarationFile && !program.isSourceFileFromExternalLibrary(file));
...
function emittedSourceDirectory(program, sourceFile) {
    const compilerOptions = program.getCompilerOptions();
    if (!compilerOptions.outDir)
        return dirname(sourceFile.fileName);
...
function generatedRuntimeSource(layouts) {
...
function prepareProject(program, options = {}) {
...
function compilePreparedProjectSync(prepared) {
    if (prepared.cacheHit)
        return;
...
export function createGeneratedProjectTransformer(program, generated, runtimeModuleSpecifier) {
    return createJsonTyTransformer(program, {
        runtimeModule: runtimeModuleSpecifier ?? ((sourceFile) => modulePath(emittedSourceDirectory(program, sourceFile), generated.runtimePath)),
 ...
 */
export function generateProjectSync(program, options = {}) {
