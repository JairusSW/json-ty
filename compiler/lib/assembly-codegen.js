import { layoutObject } from "./schema-ir.js";
...
function keyDataDeclarations(layout) {
...
function bytesMatch(pointer, value) {
    const bytes = new TextEncoder().encode(value);
    const checks = [];
    let offset = 0;
    while (offset + 16 <= bytes.length) {
...
function rawStores(pointer, bytes, indentation) {
    const stores = [];
    let offset = 0;
    while (offset + 8 <= bytes.length) {
...
function defaultInitialization(layout) {
...
function bitmapResets(layout) {
    return ["presence", "nulls", "lazy"]
        .flatMap((kind) => Array.from({ length: layout.bitmapWords }, (_, word) => `${word === 0 ? kind : `${kind}${word}`} = 0;`))
        .join("\n  ");
}
function parseField(layout, field, minified = false) {
...
function orderedParse(layout) {
    if (layout.fields.length === 0)
        return "";
    const chunkSize = layout.features.deserialize.chunkSize;
    if (chunkSize !== undefined) {
        const chunks = Array.from({ length: Math.ceil(layout.fields.length / chunkSize) }, (_, chunk) => `  if (ordered) {
...
function orderedParseHelpers(layout) {
    const chunkSize = layout.features.deserialize.chunkSize;
    if (chunkSize === undefined)
        return "";
    const chunks = [];
...
function orderedWhitespaceParse(layout) {
    if (layout.fields.length === 0)
        return "";
    const fields = layout.fields
        .map((field) => {
...
function sourcePreservesOutput(layout) {
    return layout.features.retainsSource && layout.fields.every((field) => field.defaultValue === undefined &&
...
function generateParser(layout) {
...
function serializeField(layout, field) {
...
function generateSerializer(layout) {
    const fields = layout.fields
...
function generateSimpleMaterializer(layout) {
    if (layout.lazyOffset === undefined)
        return "";
    const cases = layout.fields
        .filter(isDeferred)
        .map((field) => {
        const parse = field.kind === "number"
...
export function generateAssemblyModule(schemas, options = {}) {
    const layouts = schemas.map(layoutObject).map((layout, index) => ({
        ...layout,
        abi: {
            index,
            parse: `p${index}`,
