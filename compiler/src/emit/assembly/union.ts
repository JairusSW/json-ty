...
export const unionEmitter: AssemblyTypeEmitter<UnionTypeRef> = {
  kind: "union",
  ...
  emitSerialize(type, context) {
    const cases = type.variants
      .map(
        (variant, index) =>
          `case ${index}: if (!serialize${variant.typeName}Record(${context.document} + load<u32>(${context.source}), ${context.document})) ${context.fail} break;`,
      )
      .join("\n      ");
