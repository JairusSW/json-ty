...
@inline
export function storeDocumentHeader(document: usize, size: u32, sourceOffset: u32, sourceLength: u32, rootOffset: u32): void {
  store<u32>(document + DOCUMENT_SIZE_OFFSET, size);
  store<u32>(document + DOCUMENT_SOURCE_OFFSET, sourceOffset);
  store<u32>(document + DOCUMENT_SOURCE_LENGTH_OFFSET, sourceLength);
  store<u32>(document + DOCUMENT_ROOT_OFFSET, rootOffset);
}
