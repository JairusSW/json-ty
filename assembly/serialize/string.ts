...
import { serializeRetainedCleanString_SWAR, serializeRetainedEscapedString_SWAR } from "./swar/string";
import { serializeRetainedCleanString_SIMD, serializeRetainedEscapedString_SIMD } from "./simd/string";
...
function writeUnicodeEscape(code: u32): bool {
  return writeByte(0x5c) && writeByte(0x75) && writeByte(lowerHex((code >> 12) & 15)) && writeByte(lowerHex((code >> 8) & 15)) && writeByte(lowerHex((code >> 4) & 15)) && writeByte(lowerHex(code & 15));
}

function writeUtf8Scalar(code: u32): bool {
  if (code <= 0x7f) return writeByte(code);
  if (code <= 0x7ff) {
    return writeByte(0xc0 | (code >> 6)) && writeByte(0x80 | (code & 0x3f));
  }
  if (code <= 0xffff) {
    return writeByte(0xe0 | (code >> 12)) && writeByte(0x80 | ((code >> 6) & 0x3f)) && writeByte(0x80 | (code & 0x3f));
  }
  return writeByte(0xf0 | (code >> 18)) && writeByte(0x80 | ((code >> 12) & 0x3f)) && writeByte(0x80 | ((code >> 6) & 0x3f)) && writeByte(0x80 | (code & 0x3f));
}
...
function serializeEscapedSpan(source: usize, length: u32): bool {
  ...
  while (cursor < end) {
    ...
    if (escape == 0x75) {
      ...
      cursor += 4;
      if (code >= 0xd800 && code <= 0xdbff && cursor + 6 <= end && load<u8>(cursor) == 0x5c && load<u8>(cursor + 1) == 0x75) {
...
export function serializeString(document: usize, source: usize): bool {
  ...
  if (JSON_TY_KERNEL_TIER == 0) {
    return writeByte(0x22) && ((taggedLength & 0x80000000) == 0 ? writeRaw(<u32>(document + offset), length) : serializeEscapedSpan(document + offset, length)) && writeByte(0x22);
  }
  if (JSON_TY_KERNEL_TIER == 2 && ASC_FEATURE_SIMD) {
    return (taggedLength & 0x80000000) == 0 ? serializeRetainedCleanString_SIMD(document + offset, length) : serializeRetainedEscapedString_SIMD(document + offset, length);
  }
  return (taggedLength & 0x80000000) == 0 ? serializeRetainedCleanString_SWAR(document + offset, length) : serializeRetainedEscapedString_SWAR(document + offset, length);
}
