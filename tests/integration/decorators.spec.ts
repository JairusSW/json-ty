...
import { ObjWithStrangeKey, OmitIf, Vec3 } from "./shared-types.js";
...
describe("Should apply omitnull on nullable fields", () => {
  ...
  a.foo = "set";
  expect(JSON.stringify(a)).toBe('{"x":1,"z":1,"foo":"set"}');
});
...
describe("Should deserialize decorated player payloads with omitted fields", () => {
  ...
  expect((parsedAdult.pos as Vec3).x.toString()).toBe("9");
  expect((parsedAdult.pos as Vec3).z.toString()).toBe("7");
});
