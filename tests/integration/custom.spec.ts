...
class Point {
  ...
  serializer(self: Point): string {
    const x = Number.isInteger(self.x) ? self.x.toFixed(1) : self.x.toString();
    const y = Number.isInteger(self.y) ? self.y.toFixed(1) : self.y.toString();
...
describe("Should deserialize using custom deserializers", () => {
  ...
  expect(p1.x.toString()).toBe("1");
  expect(p1.y.toString()).toBe("2");
});
...
describe("Should deserialize nullable custom fields from values", () => {
  ...
  expect(parsed.value!.x.toString()).toBe("4");
...
describe("Should deserialize custom points with zero and negative values", () => {
  ...
  expect(parsed.x.toString()).toBe("0");
  expect(parsed.y.toString()).toBe("-3");
});
...
describe("Should deserialize multiple custom fields in one object", () => {
  ...
  expect(parsed.left.x.toString()).toBe("1");
  expect(parsed.left.y.toString()).toBe("2");
  expect(parsed.right.x.toString()).toBe("-3");
