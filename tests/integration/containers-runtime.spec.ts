...
describe("Should round-trip mixed container envelopes", () => {
  ...
  expect(parsed.staticItems[0].x.toString()).toBe("1");
  expect(parsed.staticItems[1].z.toString()).toBe("6");
...
describe("Should round-trip JSON.Value arrays containing objects, raw, and arrays", () => {
  ...
  expect(values[0].get<JSON.Obj>().get("a")!.toString()).toBe("1");
  expect(values[1].get<JSON.Arr>().at(0).toString()).toBe("2");
  expect(values[1].get<JSON.Arr>().at(1).toString()).toBe("3");
