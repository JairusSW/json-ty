...
export const CAP: i32 = 64 << 20; // 64 MiB
const BUF = new StaticArray<u8>(CAP);

// The AS-side string we hold between an ingest and a later emit.
let stored: string = "";

export function bufPtr(): usize {
...
export function ingestUtf8(len: i32): i32 {
...
export function validateUtf8(len: i32): bool {
...
export function emitUtf8(): i32 {
  return <i32>UTF8.encodeUnsafe(changetype<usize>(stored), stored.length, bufPtr());
}
